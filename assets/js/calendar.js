...
// Global state variables
        ...
        today.setHours(0,0,0,0);     // Normalize to midnight for clean date comparisons

        let currentWeekStart = getMonday(new Date(today));    // Start of the current week
        let currentMonthDate = new Date(today.getFullYear(), today.getMonth(), 1);  // 1st of current month

        let allOutfits = [];          // All saved outfits (for the picker modal)
        let weekSchedules = {};       // Schedules indexed by ISO date (e.g., "2026-03-31")
        let monthSchedules = {};      // Same format, for the month view's date range

        // Modal state
        let modalDate = null;                   // The date the user clicked on
        let modalExistingScheduleId = null;     // The schedule ID if editing an existing one
        let modalSelectedOutfitId = null;       // The outfit the user selected in the picker
        ...
        function buildWeekCards() {
            ...
            for (let i = 0; i < 7; i++) {
                ...
                if (schedule) {
                    ...
                    actionDiv.onclick = (e) => e.stopPropagation();    // Prevent card click from firing
...
// View toggle (week ↔ month)
        function toggleCalendarView() {
            ...
            if (weekView.style.display === 'none') {
                ...
                renderMonthView();    // Fetch fresh data when switching to month
            }
        }
...
// Save schedule
        function saveSchedule() {
            ...
            formData.append('scheduled_date', formatDateISO(modalDate));
