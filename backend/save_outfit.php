...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if($outfitDataString) {
        ...
        if ($stmt->execute()) {
            ...
            if ($status === 'laundry') {
                $compStmt = $conn->prepare("UPDATE clothes SET status = 'laundry' WHERE id IN (?, ?, ?) AND user_id = ?");
                $compStmt->bind_param("iiii", $top_id, $bottom_id, $shoes_id, $user_id);
                $compStmt->execute();
                $compStmt->close();
            } else if ($status === 'ready') {
                $compStmt = $conn->prepare("UPDATE clothes SET status = 'ready' WHERE id IN (?, ?, ?) AND user_id = ?");
                $compStmt->bind_param("iiii", $top_id, $bottom_id, $shoes_id, $user_id);
                $compStmt->execute();
                $compStmt->close();
            }
